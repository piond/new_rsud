...
class TbCarousel extends CWidget
{
	...
	/**
	 * @var boolean indicates whether to display the indicator links.
	 */
	...
	/**
	 * @var string location of images slide show.
	 */
	...
	public function init()
	{
		...
		if (!isset($this->prevLabel))
			$this->prevLabel='<i class="glyphicon glyphicon-chevron-left"></i>';
		
		if (!isset($this->nextLabel))
			$this->nextLabel='<i class="glyphicon glyphicon-chevron-right"></i>';
	}
	...
	public function run()
	{
		...
		if($this->displayIndicators==true)
	...
	/**
	 * Renders the carousel indicator
	...
	protected function renderIndicators($items)
	{
		...
		{
			$options=array();
			$options['data-target']='#'.$this->htmlOptions['id'];
			$options['data-slide-to']=$i;
			if($i==0) $options['class']='active';
			echo CHtml::tag('li',$options,false);
		}
		echo '</ol>';
	}
	...
	protected function renderItems($items)
	{
		foreach ($items as $i => $item)
		{
			...
			if (isset($item['image']))
			{
				...
				if(isset($this->imageLocation))
					$image=$this->imageLocation.'/'.$item['image'];
				else
					$image=$item['image'];
				echo CHtml::image($image, $item['alt'], $item['imageOptions']);
			}
