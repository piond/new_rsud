...
class TbDropdown extends TbBaseMenu
{
	...
	public function init()
	{
		...
		if(!isset($this->htmlOptions['classori'])){
	...
	protected function renderMenuItem($item)
	{
		if (isset($item['icon']))
		{
			if (strpos($item['icon'], 'icon') === false)
			{
				...
				$item['icon'] = 'glyphicon-'.implode(' glyphicon-', $pieces);
			}

			$item['label'] = '<i class="glyphicon '.$item['icon'].'"></i> '.$item['label'];
		}
