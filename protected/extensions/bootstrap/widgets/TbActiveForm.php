...
class TbActiveForm extends CActiveForm
{
	...
	// Input classes.
	const INPUT_HORIZONTAL = 'TbInputHorizontal';
	const INPUT_INLINE = 'TbInputInline';
	const INPUT_SEARCH = 'TbInputSearch';
	const INPUT_VERTICAL = 'TbInputVertical';
	...
	public $errorField=true;
	
	public static $showError;
	
	public $enter2tab = false;
	...
	protected function enterToTab() {
		Yii::app()->clientScript->registerScript('enter2tab', '
	...
	/**
	 * Renders a email field input row.
	 * @param CModel $model the data model
	 * @param string $attribute the attribute
	 * @param array $htmlOptions additional HTML attributes
	 * @return string the generated row
	...
	{
		ob_start();
		$validate=array('text','email','date','time','textarea','password','dropdownlist','filefield','captcha','uneditable');
		if(in_array($type, $validate)) isset($htmlOptions['class'])?$htmlOptions['class'].=" form-control":$htmlOptions['class']="form-control";
		$this->getOwner()->widget($this->getInputClassName(), array(
			...
			'labelWidthBox' => $this->labelwidth,
			'widthControl' => $this->formwidth 
		));
		return ob_get_clean();
	}
