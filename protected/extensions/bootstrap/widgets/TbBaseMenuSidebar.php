...
abstract class TbBaseMenuSidebar extends CMenu
{
	/**
		...
		if($n > 0)
		{
			...
			foreach ($items as $item)
			{
				...
				else
				{
					...
					if (isset($item['dropup']) && $item['dropup']===true) {
						$classes[]='dropup';
					}
					...
					{
						$itemsContainerId =  (isset($item['itemsContainerOptions']['id'])) ? $item['itemsContainerOptions']['id'] : 'dropdown1';
						$itemsContainerCollapsed =  (isset($item['itemsContainerOptions']['collapsed']) && $item['itemsContainerOptions']['collapsed'] === true) ? 'collapse' : 'in';
						//$iddiv = (isset($item['iditems'])) ? $item['iditems'] : 'dropdown1';
						//echo '<div id="'.$iddiv.'" class="accordion-body collapse">';
						//echo $itemsContainerCollapsed;
						echo '<div id="'.$itemsContainerId.'" class="'.$itemsContainerCollapsed.'">';
	...
	protected function renderMenuItem($item)
	{
		if (isset($item['icon']))
		{
			if (strpos($item['icon'], 'icon') === false)
			{
				...
				$item['icon'] = 'fa-'.implode(' icon-', $pieces);
			}

			$item['label'] = '<i class="fa '.$item['icon'].'"></i> '.$item['label'];
		}
		...
		if (isset($item['items']) && !empty($item['items']))
		{
			...
			$item['linkOptions']['data-toggle'] = isset($item['linkOptions']['data-toggle']) ? $item['linkOptions']['data-toggle'] : 'dropdown';
